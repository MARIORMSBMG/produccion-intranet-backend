...
app.post('/registro', auth, (req, res) => {
  ...
  const stmt = db.prepare("INSERT INTO produccion (nomina, nombre, area, fecha, hora, proyecto, codigo, estandar, cantidad, tiempo_interferencia, motivo_interferencia, promedio_eficiencia, total_piezas, eficiencia_ajustada, turno, maquina, tipo_interferencia) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
...
app.get('/resumen', auth, (req, res) => {
  ...
  db.all("SELECT nomina, nombre, SUM(cantidad) AS total_piezas, ROUND(AVG(promedio_eficiencia), 1) AS eficiencia_prom, ROUND(AVG(eficiencia_ajustada), 1) AS eficiencia_ajustada, SUM(tiempo_interferencia) AS total_interferencia FROM produccion WHERE fecha = ? GROUP BY nomina", [fecha], (err, rows) => {
