...
// Registro de producción con eficiencia ajustada
app.post('/registro', auth, (req, res) => {
  ...
  const stmt = db.prepare(`INSERT INTO produccion (
    ...
    promedio_eficiencia, total_piezas, eficiencia_ajustada
  ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`);
  for (const d of datos) {
    stmt.run([
      ...
      d.promedio_eficiencia, d.total_piezas, d.eficiencia_ajustada
    ]);
  }
  stmt.finalize();
  res.json({ success: true });
